...
package com.news.hr.framework.poi.xlsx;
...
import com.news.hr.framework.poi.pojo.ExcelProperty;
import com.news.hr.framework.poi.util.DateUtil;
import com.news.hr.framework.poi.util.POIUtil;
import com.news.hr.framework.poi.util.ValidatorUtil;
...
public class ExcelXlsxWriter {
  ...
  public SXSSFWorkbook generateXlsxWorkbookByTemplate(List<?> data,int startRow,int startCol,String templateName) {
//	  URL url = ExcelXlsxWriter.class.getClassLoader().getResource(TEMPLATE_PATH);
//	  if(url == null) {
//		  throw new ExcelKitAnnotationAnalyzeException("模板路径:/src/main/resources/"+TEMPLATE_PATH+"不存在!");
//	  }
//	  String path = url.getPath()+File.separator+templateName;
	  String path = TEMPLATE_PATH+templateName;
	  SXSSFWorkbook workbook = POIUtil.newSXSSFWorkbook(path);
	  ...
	  for(int i=0;i<data.size();i++) {
		  SXSSFRow bodyRow = POIUtil.newSXSSFRow(sheet, startRow++);
  ...
  private SXSSFSheet generateXlsxHeader(SXSSFWorkbook workbook,
    ...
    for (int i = 0; i < propertyList.size(); i++) {
      ...
      if (isTemplate) {
        // cell range
    	//**************2019.06.12 50唐 start**************//
        POIUtil.setColumnCellRangeNew(workbook,sheet, property.getOptions(), 1, mMaxSheetRecords, i, i);
        //**************2019.06.12 50唐 end**************//
        // cell comment.
