 ...
 */
@RestController
@Api(value = "system-File", tags = {"system-File"})
@RequestMapping("file")
public class FileController {
    ...
    */
    @ApiOperation(value = "保存数据", notes = "保存数据到File",httpMethod = "POST")
    @PostMapping(value = "/save", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ReturnModel save(@RequestBody FileForm fileForm) {
        Integer result = fileService.save(fileForm);
     ...
     */
    @ApiOperation(value = "删除数据", notes = "根据主键id删除File数据",httpMethod = "DELETE")
    @DeleteMapping(value = "/deleteById/fileId", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ReturnModel deleteById(@ApiParam(name = "fileId",value = "file主键id")
            @PathVariable String fileId) {
        Integer result = fileService.deleteById(fileId);
        return ReturnModel.newSuccessInstance(result);
    }
    ...
    */
    @ApiOperation(value = "更新数据", notes = "根据主键id更新File数据",httpMethod = "PUT")
    @PutMapping(value = "/updateById/fileId", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ReturnModel updateById(@RequestBody FileForm fileForm) {
        Integer result = fileService.updateById(fileForm);
        return ReturnModel.newSuccessInstance(result);
    }
    ...
    */
    @ApiOperation(value = "获取单条数据", notes = "根据主键id获取File数据",httpMethod = "GET")
    @GetMapping(value = "/selectById/fileId", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ReturnModel selectById(@RequestParam(value = "fileId")  String fileId) {
        FileVo result = fileService.selectById(fileId);
        return ReturnModel.newSuccessInstance(result);
    }
     ...
     */
    @ApiOperation(value = "分页获取所有file信息", notes = "获取所有file信息",httpMethod = "GET")
    @ApiImplicitParams({
    ...
    public ReturnModel selectPage(FileQuery fileQuery){
        IPage<FileVo> pageList=fileService.selectPage(fileQuery);
    ...
    public void downloadTemplate(HttpServletResponse response) {
        ...
        ExcelKit.$Export(FileImport.class, response).downXlsx(fileImports, true);
    }
    ...
    public ReturnModel importData(@RequestParam(value = "file") MultipartFile file) throws IOException {
        List<FileImport> successResult = Lists.newArrayList();
        List<Map<String, Object>> errorResult = Lists.newArrayList();
        ExcelKit.$Import(FileImport.class).readXlsx(file.getInputStream(), new ExcelReadHandler<FileImport>() {
            @Override
            public void onSuccess(int sheetIndex, int rowIndex, FileImport entity) {
        ...
        List<File> files = FileConvert.convertToPoByImport(successResult);
    ...
    public void exportData(HttpServletResponse response, FileQuery fileQuery) {
        List<FileVo> fileVos = fileService.selectList(fileQuery);

        List<FileExport> fileExports = FileConvert.convertToExportByVo(fileVos);
        ExcelKit.$Export(FileExport.class, response).downXlsx(fileExports, false);
    }
